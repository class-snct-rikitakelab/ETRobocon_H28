...
public class GarageSolver {
    ...
    static int distanceToGarage = 60;          //ガレージに進入するために前進する距離cm単位
    static int tailangle = 90;                 //三点倒立するためのしっぽの角度
    private static int Motor_Power = 30;   //モータの基礎的なPWM値
    ...
    private static int PowerL = Motor_Power;
    private static int PowerR = Motor_Power;
    ...
    public static void SolveGarage(){
    ...
    public static void BeforeTakeInto(){       //進入待ち状態にはいるためのメソッド 三点倒立で1秒間静止
    ...
    public static void TakeInto(){                                     //distanceToGarageで定めた距離だけ前進
    ...
    public static void KeepSec(){  //経過待ち状態の時のメソッド 完全停止状態で3秒間維持

        int currentPower = Motor_Power;

        //float start = getDistance(Hardware);
        ...
        //float end = getDistance(Hardware);
